...
class ProzedaReader(object):
    """Reader for the data on the wire converted by the microcontroller"""

    def __init__(self, serialport, logcount=10, dispcount=10):
        self.ser = serial.Serial(serialport, 115200)
        self.thread_serial = threading.Thread(target=self.run, args=())
        self.thread_serial.daemon = True

        #self.thread_monitor = None
        #self.thread_monitor = threading.Thread(target=self.monitor_tick, args=())
        self.readerrunning = True
        self.rx_error_count = 0
        self.last_error = None
        self.hist_logdata = [] #TODO: this is quite inefficient, change to collections.deque
        self.hist_dispdata = [] #TODO: this is quite inefficient, change to collections.deque
        #self.hist_commerror = [] #TODO: this is quite inefficient, change to collections.deque

        self.histlen_logdata = logcount
        self.histlen_dispdata = dispcount
        #self.histlen_commerror = 10
        self.logdata_timeout = 10
    ...
    def rx_logdata(self, data, timestamp):
        ...
        self.hist_logdata = self.hist_logdata[-self.histlen_logdata:]
    ...
    def rx_displaydata(self, data, timestamp):
        ...
        self.hist_dispdata = self.hist_dispdata[-self.histlen_dispdata:]
    ...
    def monitor_tick(self):
        """monitors incoming data, if no data at all received or the timeout for logdata
        (logdata_timeout) elapsed, an error will be raised every 2 seconds"""
        while self.readerrunning:
            time.sleep(2)
            if len(self.hist_logdata) == 0:
                self.rx_data_missing(time.time())
            elif self.hist_logdata[-1].timestamp < (time.time() - self.logdata_timeout):
                self.rx_data_missing(time.time())
